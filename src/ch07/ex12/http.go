...
func (db *database) list(w http.ResponseWriter, req *http.Request) {
	db.withRLockContext(func() {
		...
		requestTemplate().Execute(w, response(db))
	})
}
...
func response(data *database) DBList {
	list := DBList{}
	for name, price := range data.products {
		db := DB{name, float32(price)}
		list.Lists = append(list.Lists, db)
	}
	return list
}
